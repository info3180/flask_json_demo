...
This is a very simple demo of using flask.jsonify() to serve a JSON based view
It isn't very useful. Consider it a starting point.

Here's a checklist of things you might want to consider regarding this code.

- consider tying it to a database
- add a login form so that the "current" user IS the logged in user
- consider making the _get_current_user into a query that returns a user in json format
...
Clone the repo.
    ...
    git clone git@github.com:info3180/flask_json_demo.git
    cd flask_json_demo
